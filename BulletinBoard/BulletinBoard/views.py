...
class UserPostListView(ListView):
    model = Bulletin
    # <app>/<model>_<viewtype>.html
    template_name = 'flatpages/user_posts.html'
    # this variable <context_object_name> is passed to the template --> home.html
    context_object_name = 'bulletins_0'
    ordering = ['-date_created']
    paginate_by = 2
...
class PostDetailView(DetailView):
    model = Bulletin
    template_name = 'flatpages/bulletin_detail.html'


class PostCreateView(LoginRequiredMixin, CreateView):
    model = Bulletin
    fields = ['title', 'content', 'bulletin_category', 'file']
    template_name = 'flatpages/bulletin_form.html'
...
class CommentCreateView(LoginRequiredMixin, CreateView):
    ...
    template_name = 'flatpages/comment_form.html'
...
class PostDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    model = Bulletin
    template_name = 'flatpages/bulletin_confirm_delete.html'  # Убедитесь, что путь к шаблону правильный
    success_url = reverse_lazy('home')  # Перенаправление после успешного удаления

    # Проверка, является ли пользователь автором
    def test_func(self):
        bulletin = self.get_object()
        return self.request.user == bulletin.author  # Если пользователь - автор, возвращаем True


class CommentListView(LoginRequiredMixin, ListView):
    model = Comment
    # <app>/<model>_<viewtype>.html
    template_name = 'flatpages/comment_list.html'
